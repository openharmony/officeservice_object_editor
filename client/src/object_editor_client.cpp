...
sptr<IObjectEditorManager> ObjectEditorClient::GetIObjectEditorManager()
{
    ...
    OBJECT_EDITOR_LOGI(ObjectEditorDomain::CLIENT, "Notify samgr load sa %{public}d success", OBJECT_EDITOR_SERVICE_SA_ID);
    return oeSAProxy_;
}
...
{
    std::unique_lock<std::mutex> lock(loadMutex_);
    auto wait = loadCond_.wait_for(lock, std::chrono::seconds(LOAD_TIMEOUT_MS), [this] { return loadState_== true; });
    if(!wait){
...
void ObjectEditorLoadCallback::OnLoadSystemAbilitySuccess(int32_t systemAbilityId, const sptr<IRemoteObject> &remoteObject)
{
    OBJECT_EDITOR_LOGD(ObjectEditorDomain::CLIENT, "in");
    if (systemAbilityId != OBJECT_EDITOR_SERVICE_SA_ID) {
    ...
    OBJECT_EDITOR_LOGI(ObjectEditorDomain::CLIENT, "OnLoadSystemAbilitySuccess, systemAbilityId: %{public}d", systemAbilityId);
...
void ObjectEditorLoadCallback::OnLoadSystemAbilityFail(int32_t systemAbilityId)
{
    ...
    OBJECT_EDITOR_LOGE(ObjectEditorDomain::CLIENT, "OnLoadSystemAbilityFailed, systemAbilityId: %{public}d", systemAbilityId);
...
std::string ObjectEditorClient::GenRandomUuid()
{
    ...
    uuid_[15] = static_cast<uint8_t>(static_cast<uint64_t>(tv.tv_usec) & 0x00000000000000FF);
    uuid_[14] = static_cast<uint8_t>((static_cast<uint64_t>(tv.tv_usec) & 0x000000000000FF00) >> 8);
    uuid_[13] = static_cast<uint8_t>((static_cast<uint64_t>(tv.tv_usec) & 0x0000000000FF0000) >> 16);
    uuid_[12] = static_cast<uint8_t>((static_cast<uint64_t>(tv.tv_usec) & 0x00000000FF000000) >> 24);
    uuid_[10] = static_cast<uint8_t>((static_cast<uint64_t>(tv.tv_usec) & 0x000000FF00000000) >> 32);
    uuid_[9] = static_cast<uint8_t>((static_cast<uint64_t>(tv.tv_usec) & 0x0000FF0000000000) >> 40);
    uuid_[8] = static_cast<uint8_t>((static_cast<uint64_t>(tv.tv_usec) & 0x00FF000000000000) >> 48);
    uuid_[7] = static_cast<uint8_t>((static_cast<uint64_t>(tv.tv_usec) & 0xFF00000000000000) >> 56);
    uuid_[6] = static_cast<uint8_t>((unsigned int)randomTime.tm_sec + static_cast<unsigned int>(randomNum) & 0xFF);
    uuid_[5] = static_cast<uint8_t>(((unsigned int)randomTime.tm_min + (randomNum >> 8)) & 0xFF);
    uuid_[4] = static_cast<uint8_t>(((unsigned int)randomTime.tm_hour + (randomNum >> 16)) & 0xFF);
    uuid_[3] = static_cast<uint8_t>(((unsigned int)randomTime.tm_mday + (randomNum >> 24)) & 0xFF);
    uuid_[2] = static_cast<uint8_t>((unsigned int)randomTime.tm_mon & 0xFF);
    uuid_[1] = static_cast<uint8_t>((unsigned int)randomTime.tm_year & 0xFF);
    uuid_[0] = static_cast<uint8_t>(((unsigned int)randomTime.tm_year & 0xFF00) >> 8);
    std::string tmp = "";
    std::string ret = "";
    ...
    if(context == nullptr || document == nullptr)
{
    ...
    std::string snapshotFilePath = sandboxPath.string() + "/snapshot.png";
...
ErrCode ObjectEditorClient::GetObjectEditorFormatByHmidAndLocale(const std::string &hmid, const std::string &locale,
        std::unique_ptr<ObjectEditorFormat> &format)
{
    ...
    if(ret != ERR_OK){
        OBJECT_EDITOR_LOGE(ObjectEditorDomain::CLIENT, "GetObjectEditorFormatByHmidAndLocale failed, ret is %{public}d", ret);
        return ret;
    }
    return ERR_OK;
}
        ...
        std::vector<std::unique_ptr<ObjectEditorFormat>> &formats)
{
    ...
    if(ret != ERR_OK){
        OBJECT_EDITOR_LOGE(ObjectEditorDomain::CLIENT, "GetObjectEditorFormatsByLocale failed, ret is %{public}d", ret);
        return ret;
    }
    return ERR_OK;
}
