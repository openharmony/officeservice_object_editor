...
#include "oh_contentembed_proxy_registeronextensionstoppedfunc.h"
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    if (provider.ConsumeBool()) {
        func = [](ContentEmbed_ExtensionProxy *proxy) {};
    }
    if (provider.ConsumeBool()) {
        OH_ContentEmbed_Proxy_RegisterOnExtensionStoppedFunc(proxy, func);
    } else {
        OH_ContentEmbed_Proxy_RegisterOnExtensionStoppedFunc(nullptr, func);
    }
    delete proxy;
    return true;
}
