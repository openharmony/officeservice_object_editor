...
#include "oh_contentembed_destroycontentembedformat.h"
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    if (provider.ConsumeBool()) {
        ContentEmbed_Format *info = new ContentEmbed_Format();
        OH_ContentEmbed_DestroyContentEmbedFormat(info);
    } else {
        OH_ContentEmbed_DestroyContentEmbedFormat(nullptr);
    }
    return true;
}
}
