...
#include "oh_contentembed_getfilenameextensionsfromformat.h"
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    if (provider.ConsumeBool()) {
        OH_ContentEmbed_GetFileNameExtensionsFromFormat(&info, &count);
    } else {
        OH_ContentEmbed_GetFileNameExtensionsFromFormat(nullptr, &count);
    }
    return true;
}
}
