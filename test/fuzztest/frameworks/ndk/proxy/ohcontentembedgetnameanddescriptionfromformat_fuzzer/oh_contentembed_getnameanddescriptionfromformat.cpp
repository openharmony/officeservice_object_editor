...
#include "oh_contentembed_getnameanddescriptionfromformat.h"

using namespace OHOS::ObjectEditor;
namespace {
constexpr float IS_CONFIG = 0.2;
constexpr int32_t MUL = 5;
}
namespace OHOS {
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    int32_t length = provider.ConsumeIntegralInRange<int32_t>(0, MAX_NAME_LENGTH) * MUL;
    char *name = new char[length];
    int32_t descriptionLength = provider.ConsumeIntegralInRange<int32_t>(0, MAX_DESCRIPTION_LENGTH) * MUL;
    char *description = new char[descriptionLength];
    if (provider.ConsumeBool()) {
        OH_ContentEmbed_GetNameAndDescriptionFromFormat(&info, name, description);
    } else {
        OH_ContentEmbed_GetNameAndDescriptionFromFormat(nullptr, name, description);
    }
    delete[] name;
    delete[] description;
    return true;
