...
#include "oh_contentembed_proxy_registeroneditingfinishedfunc.h"
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    if (provider.ConsumeBool()) {
        func = [](ContentEmbed_ExtensionProxy *proxy, bool dataModified) {};
    }
    if (provider.ConsumeBool()) {
        OH_ContentEmbed_Proxy_RegisterOnEditingFinishedFunc(proxy, func);
    } else {
        OH_ContentEmbed_Proxy_RegisterOnEditingFinishedFunc(nullptr, func);
    }
    delete proxy;
    return true;
}
