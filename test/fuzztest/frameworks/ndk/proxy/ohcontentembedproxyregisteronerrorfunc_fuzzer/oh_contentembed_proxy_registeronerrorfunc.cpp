...
#include "oh_contentembed_proxy_registeronerrorfunc.h"
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    if (provider.ConsumeBool()) {
        func = [](ContentEmbed_ExtensionProxy *proxy, ContentEmbed_ErrorCode error) {};
    }
    if (provider.ConsumeBool()) {
        OH_ContentEmbed_Proxy_RegisterOnErrorFunc(proxy, func);
    } else {
        OH_ContentEmbed_Proxy_RegisterOnErrorFunc(nullptr, func);
    }
    delete proxy;
    return true;
}
