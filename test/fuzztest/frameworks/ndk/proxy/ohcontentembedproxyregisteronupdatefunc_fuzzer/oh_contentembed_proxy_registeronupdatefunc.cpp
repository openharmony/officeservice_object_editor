...
#include "oh_contentembed_proxy_registeronupdatefunc.h"
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    if (provider.ConsumeBool()) {
        OH_ContentEmbed_Proxy_RegisterOnUpdateFunc(proxy, func);
    } else {
        OH_ContentEmbed_Proxy_RegisterOnUpdateFunc(nullptr, func);
    }
    delete proxy;
    return true;
}
