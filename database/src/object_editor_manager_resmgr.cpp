...
{
    OBJECT_EDITOR_LOGI(ObjectEditorDomain::DATABASE, "bundleName:%{private}s moduleName:%{private}s locale:%{private}s",
...
{
    OBJECT_EDITOR_LOGI(ObjectEditorDomain::DATABASE, "bundleName:%{private}s", bundleName.c_str());
    std::lock_guard<std::mutex> lock(resMgrMutex_);
