...
ContentEmbed_ErrorCode CopyStream(ContentEmbed_Storage *srcStorage, ContentEmbed_Storage *dstStorage, const char *name)
{
    ...
    err = OH_ContentEmbed_Storage_GetStream(srcStorage, name, &srcStream);
    if (err != CE_ERR_OK) {
        return err;
    }
    ...
    ContentEmbed_StorageElements *elements, size_t index)
{
    ...
    auto err = OH_ContentEmbed_StorageElement_GetElement(elements, index, &element);
    if (err != CE_ERR_OK) {
        return err;
    }
    ...
    if (isStorage) {
        ...
        err = OH_ContentEmbed_StorageElement_GetStorage(srcStorage, name, &srcSubStorage);
        if (err != CE_ERR_OK) {
            return err;
        }
        err = OH_ContentEmbed_StorageElement_CreateStorage(dstStorage, name, &dstSubStorage);
        if (err != CE_ERR_OK) {
            delete srcSubStorage;
            srcSubStorage = nullptr;
            return err;
        }
        err = OH_ContentEmbed_StorageElement_CopyTo(dstSubStorage, srcSubStorage);
        delete srcSubStorage;
        srcSubStorage = nullptr;
...
ContentEmbed_ErrorCode CopyElements(ContentEmbed_Storage *srcStorage, ContentEmbed_Storage *dstStorage)
{
    ...
    err = OH_ContentEmbed_StorageElement_GetElements(srcStorage, elements);
    if (err != CE_ERR_OK) {
    ...
    size_t count = 0;
    err = OH_ContentEmbed_StorageElement_GetCount(elements, &count);
    if (err != CE_ERR_OK) {
        OH_ContentEmbed_StorageElements_Destroy(elements);
        return err;
    }
