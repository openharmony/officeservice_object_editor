 ...
 * 如果object editor服务进程尚未启动，则object editor服务启动时会向**BMS**查询所有已安装应用的ExtensionAbility信息。如果该信息中的`type`属性值为`contentEmbed`，即为OE Extension信息，object editor服务会解析`content_embed_config.json`配置文件，将**HMID**、OE服务端应用包名、文档格式后缀、应用图标、文档格式名称以及文档格式描述等信息存入数据库中。其中HMID作为关键字段，用于在后续OE文档需拉起OE服务端应用进行编辑时，通过其定位应启动的OE服务端应用。定位方法为：以**HMID**为关键字段，在数据库中搜索对应的OE服务端应用包名，以确定需启动的OE服务端应用。
