...
该部件由**框架层**与**服务层**两部分组成：
* **框架层**：为客户端应用和服务端应用所需的开发接口。客户端应用可以通过该接口在自身文档中嵌入服务端应用的文档；服务端应用则利用框架提供的能力，向客户端应用提供特定格式文档的嵌入与编辑支持。
* **服务层**：负责响应服务端应用的注册请求，并在客户端应用发起请求时启动相应的服务端应用，同时管理其生命周期，保障嵌入与编辑流程的稳定运行。
...
4. **HMID**：全称Harmony ID，是**OE文档**的系统可识别标识符，包含在文档中。系统通过HMID定位并加载支持该OE文档的服务端应用，从而实现编辑功能。
...
* **应用层**
  * **客户端应用**：指作为被嵌入文档的宿主、面向终端用户的应用。通过调用OE框架层接口实现业务逻辑，如嵌入外部文档、展示文档快照，以及按需拉起服务端应用以触发文档编辑操作。
  * **服务端应用**：指为客户端应用提供特定格式文档嵌入能力的终端用户应用。该应用需向系统注册OE Extension组件，运行时由系统根据客户端请求动态拉起，以响应文档嵌入与编辑的相关操作。
  * 如在CAD文档中嵌入Excel表格的场景下，支持编辑CAD文档的应用作为客户端应用，负责嵌入并展示外部文档内容。支持编辑Excel表格的应用则作为服务端应用，由系统按需拉起，提供文档编辑能力。

* **框架层**
  * **OE Client**：负责提供客户端应用所需的接口及框架实现，主要包括 `OH_ContentEmbed_CreateExtensionProxy` 和 `OH_ContentEmbed_DestroyExtensionProxy` 等，用于与**服务层**交互。
  * **OE Document**：负责提供OE文档读写能力的API接口及框架实现，支持客户端与服务端应用按照OE文档标准格式生成和解析文档数据。
  * **OE Extension**：负责提供OE Extension组件的开发接口及框架实现，供服务端应用接入，实现文档嵌入与拉起编辑的业务能力。
...
系统服务层作为OE能力的核心调度和管理模块，运行与独立服务进程中，负责与系统底层能力交互，并对上层框架提供支撑。其内部模块划分如下：
  * **OE 客户端会话管理**：负责响应客户端应用的 IPC 请求，管理跨进程通信会话的创建、维护与销毁。
  ...
  * **BMS (Bundle Manager Service)**：包管理服务，为object editor服务提供已安装应用中注册的OE Extension信息查询能力，支持OE Extension信息的动态增加、修改和删除。
...
2. **建立会话**：
   * 服务启动后，客户端通过 IPC 与系统服务层中的 **OE 客户端会话管理** 模块建立连接，系统为该客户端分配对应的服务资源。
3. **资源回收**：
   * **主动销毁**：当 **客户端应用** 调用 `OH_ContentEmbed_Proxy_StopWork` 时，object editor服务将停止对应的OE Extension并释放相关资源。
   * **异常监测**：在建立连接过程中，客户端会将回调对象（Stub）注册至object editor服务。 **OE 客户端会话管理** 模块通过 IPC 机制订阅该对象的[**断开回调（Death Recipient）**](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/ipc/subscribe-remote-state.md)；一旦感知客户端异常终止，服务将立即清理该客户端占用的会话资源，并停止对应的OE Extension。
...
服务端应用配置OE Extension的参考示例见[配置示例](#服务端配置oe-extensionability示例)。
 * 在应用安装、卸载或更新时，**BMS** 会存储、删除或更新应用包的相关信息，而应用包信息中会包含ExtensionAbility的详细信息。
 * 如果object editor服务进程尚未启动，则object editor服务启动时会向**BMS**查询所有已安装应用的ExtensionAbility信息。如果该信息中的`type`属性值为`contentEmbed`，即为OE Extension信息，object editor服务会解析`content_embed_config.json`配置文件，将**HMID**、服务端应用包名、文档格式后缀、应用图标、文档格式名称以及文档格式描述等信息存入数据库中。其中HMID作为关键字段，用于在后续OE文档需拉起服务端应用进行编辑时，通过其定位应启动的服务端应用。
 * 若object editor服务进程已启动，则object editor服务会监听BMS发送的事件通知，从而对数据中单个应用的OE Extension信息进行增加、修改或删除操作。
...
2. **拉起OE Extension**：
   * **客户端应用** 首先通过 `OH_ContentEmbed_CreateExtensionProxy` 创建OE Extension代理，随后通过该代理调用 `OH_ContentEmbed_Proxy_StartWork` 通知**object editor 服务**拉起OE Extension组件。在多个服务端应用注册了相同的文档格式后缀的场景中，存在两个决定选择哪个服务端应用的策略：
     - **策略一**：检查备选的多个服务端应用中是否包含对应文档格式后缀的默认打开应用，如果有，则选择默认应用。
     - **策略二**：当策略一无法选择时，优先选择最新注册的服务端应用。
3. **服务端应用被拉起及注册OE Extension回调**：
   * **object editor 服务** 通过**AMS**接口拉起服务端应用进程，此过程会执行OE Extension的入口函数`OH_AbilityRuntime_OnNativeExtensionCreate`。在该函数中，服务端应用需注册OE Extension的回调函数以响应客户端请求。主要的回调函数包括：`OH_ContentEmbed_Extension_RegisterOnCreateFunc`、`OH_ContentEmbed_Extension_RegisterOnDestroyFunc`、`OH_ContentEmbed_Extension_RegisterOnGetSnapshotFunc`以及其它必要回调。
4. **客户端获取OE文档快照**：
   * **客户端应用** 通过调用 `OH_ContentEmbed_Proxy_GetSnapshot` 与OE Extension进行通信，以获取OE文档的快照。
...
1. **客户端通知服务端编辑OE文档**：
   * 若OE Extension已被拉起，则 **客户端应用** 调用 `OH_ContentEmbed_Proxy_DoEdit` 与 OE Extension通信，以此通知服务端编辑OE文档。
   * 若OE Extension未被拉起，则 **客户端应用** 需先通过调用 `OH_ContentEmbed_Proxy_StartWork`拉起OE Extension。
   * 当支持编辑OE文档的服务端应用未被安装时，已嵌入OE文档的宿主文档仍可被客户端应用正常打开，并且OE文档的快照可见，但无法通过服务端应用进行OE文档的编辑和更新，也无法通过其它支持该文档格式编辑的应用进行打开编辑。
2. **服务端拉起UIAbility编辑OE文档**：
   * 当 **服务端应用** 的`OnDoEditFunc`回调被触发时，服务端应用需首先调用 `OH_ContentEmbed_Extension_GetContentEmbedContext` 获取OE Extension的上下文Context，然后通过该Context调用 `OH_ContentEmbed_Extension_ContextStartSelfUIAbility`向**AMS**发送请求，通知其拉起UIAbility组件，用户随后可在UIAbility组件中编辑OE文档。
3. **客户端刷新OE文档快照**：
   * 当OE文档内容更新时，**服务端应用** 会调用 `OH_ContentEmbed_Extension_CallbackToOnUpdate` 通知 **客户端应用**刷新当前快照。
   * 当 **客户端应用** 收到此项回调通知后，应主动调用`OH_ContentEmbed_Proxy_GetSnapshot`获取OE文档的最新快照，并刷新页面来反映更新。
...
> 该部件是默认会被编译，如果不想编译该部件，需在平台的编译配置文件中移除对应的部件信息，例如：`rk3568/config.json`。
...
从API version24开始，开发者可以在`module.json5`文件的`extensionAbilities`标签中配置OE ExtensionAbility，需要将`type`属性配置成`contentEmbed`，并且在`metadata`中增加`content_embed_config`配置文件。示例如下：
```
"extensionAbilities": [
    {
        "name": "OEExtAbility",
...
- 如果服务端应用在其它操作系统上也有相同功能，建议复用已在其它系统中使用的ID；
- 如果服务端应用该功能仅在OpenHarmony系统上提供，则建议使用业界通用的UUID工具生成新的ID。
