...
* **服务层**：负责响应OE服务端应用的注册请求，并在OE客户端应用发起请求时启动相应的OE服务端应用，同时管理其生命周期，保障嵌入与编辑流程的稳定运行。
...
4. **HMID**：全称Harmony ID，是**OE文档**的系统可识别标识符，包含在文档中。系统通过HMID定位并加载支持该OE文档的OE服务端应用，从而实现编辑功能。
...
* **应用层**
  ...
  * **OE服务端应用**：指为OE客户端应用提供特定格式文档嵌入能力的终端用户应用。该应用需向系统注册OE Extension组件，运行时由系统根据客户端请求动态拉起，以响应文档嵌入与编辑的相关操作。
  * 如在CAD文档中嵌入Excel表格的场景下，支持编辑CAD文档的应用作为OE客户端应用，负责嵌入并展示外部文档内容。支持编辑Excel表格的应用则作为OE服务端应用，由系统按需拉起，提供文档编辑能力。

* **框架层**
  * **OE Client**：负责提供OE客户端应用所需的接口及框架实现，主要包括 `OH_ContentEmbed_CreateExtensionProxy` 和 `OH_ContentEmbed_DestroyExtensionProxy` 等，用于与**服务层**交互。
...
1. **拉起服务**：
   * 当**object editor 服务**未启动时，**OE客户端应用**可通过两类接口启动服务：
...
3. **资源回收**：
   * **主动销毁**：当 **OE客户端应用** 调用 `OH_ContentEmbed_Proxy_StopWork` 时，object editor服务将停止对应的OE Extension并释放相关资源。
 ...
 * 如果object editor服务进程尚未启动，则object editor服务启动时会向**BMS**查询所有已安装应用的ExtensionAbility信息。如果该信息中的`type`属性值为`contentEmbed`，即为OE Extension信息，object editor服务会解析`content_embed_config.json`配置文件，将**HMID**、OE服务端应用包名、文档格式后缀、应用图标、文档格式名称以及文档格式描述等信息存入数据库中。其中HMID作为关键字段，用于在后续OE文档需拉起OE服务端应用进行编辑时，通过其定位应启动的OE服务端应用。定位方法为：以**HMID**为key，在数据库中搜索对应的OE Extension信息，通过OE Extension信息中包含的OE服务端应用包名确定需启动的OE服务端应用。
...
1. **创建OE文档**：
   * 当 **OE客户端应用** 调用 `OH_ContentEmbed_CreateDocumentByHmid/OH_ContentEmbed_CreateDocumentByFile` 时，**OE Document** 模块会创建OE文档，并将该文档对象返回 **OE客户端应用**。
2. **拉起OE Extension**：
...
4. **客户端刷新OE文档快照**：
   * 当OE文档内容编辑保存后，**OE服务端应用** 会调用 `OH_ContentEmbed_Extension_CallbackToOnUpdate` 通知 **OE客户端应用**刷新当前快照。
   * 当 **OE客户端应用** 收到此项回调通知后，应主动调用`OH_ContentEmbed_Proxy_GetSnapshot`获取OE文档的最新快照，并刷新页面来反映更新。
