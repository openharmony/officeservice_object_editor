...
4. **OEID**：**OE文档**的系统可识别标识符，包含在文档中。系统通过OEID定位并加载支持该OE文档的OE服务端应用，从而实现编辑功能。
...
1. **拉起服务**：
   * 当**object editor 服务**未运行的情况下，**OE客户端应用**可通过两类接口实现服务的启动：
     * 查询已注册的OE Extension信息接口：`OH_ContentEmbed_GetContentEmbedInfo`，`OH_ContentEmbed_GetContentEmbedFormatByOEidAndLocale`。
...
1. **创建OE文档**：
   * 当 **OE客户端应用** 调用 `OH_ContentEmbed_CreateDocumentByOEid/OH_ContentEmbed_CreateDocumentByFile` 时，**OE Document** 模块会创建OE文档，并将该OE文档对象返回 **OE客户端应用**。
2. **拉起OE Extension**：
