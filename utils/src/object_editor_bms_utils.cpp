...
bool ObjectEditorBmsUtils::QueryAbilityInfo(const AAFwk::Want &want, AppExecFwk::AbilityInfo &abilityInfo)
{
    ...
    bool ret = bundleMgr->QueryAbilityInfo(want, abilityInfoFlag, UserMgr::GetInstance().GetUserId(), abilityInfo);
    return ret;
}

sptr<AppExecFwk::IBundleMgr> ObjectEditorBmsUtils::GetBundleMgr()
{
    OBJECT_EDITOR_LOGD(ObjectEditorDomain::COMMON, "in");
    sptr<ISystemAbilityManager> saMgr = SystemAbilityManagerClient::GetInstance().GetSystemAbilityManager();
    if (saMgr == nullptr) {
        OBJECT_EDITOR_LOGE(ObjectEditorDomain::COMMON, "get sa fail");
        return nullptr;
    }
    sptr<IRemoteObject> remoteObject = saMgr->GetSystemAbility(BUNDLE_MGR_SERVICE_SYS_ABILITY_ID);
    if (remoteObject == nullptr) {
        OBJECT_EDITOR_LOGE(ObjectEditorDomain::COMMON, "get bundle fail");
        return nullptr;
    }
